...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 04, 2019 at 03:04 PM
...
CREATE TABLE `coupons` (
  ...
  `type` varchar(1) DEFAULT NULL COMMENT 'Valid Values :($,%)',
...
CREATE TABLE `invoices` (
  ...
  `transactions_id` varchar(45) DEFAULT NULL,
...
CREATE TABLE `plans` (
  ...
  `plan_type` varchar(1) DEFAULT NULL COMMENT 'Valid Values:\nM=Month\nY=Year\nL=Lifetime\n',
  `status` int(1) NOT NULL DEFAULT '1' COMMENT '1 = active\n0 = inactive',
  `analytics_code` mediumtext,
  `thanks_page_analytics_code` mediumtext,
...
CREATE TABLE `products` (
  ...
  `email_support` varchar(64) NOT NULL DEFAULT '0',
  `created` datetime DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
ALTER TABLE `items`
  ADD PRIMARY KEY (`id`),
  ADD KEY `fk_items_invoices1_idx` (`invoices_id`);
...
--
ALTER TABLE `items`
  ADD CONSTRAINT `fk_items_invoices1` FOREIGN KEY (`invoices_id`) REFERENCES `invoices` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
